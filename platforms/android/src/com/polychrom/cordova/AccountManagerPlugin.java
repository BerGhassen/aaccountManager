...
public class AccountManagerPlugin extends CordovaPlugin
{
	...
	public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException
	{
		...
		try
		{
			...
			else if("getAuthenticatorTypes".equals(action))
			{
				AuthenticatorDescription[] ads = manager.getAuthenticatorTypes();
				JSONArray result = new JSONArray();

				for(AuthenticatorDescription ad: ads)
				{
					JSONObject ad_object = new JSONObject();
					ad_object.put("packageName", ad.packageName);
					ad_object.put("type", ad.type);
					result.put(ad_object);
				}
